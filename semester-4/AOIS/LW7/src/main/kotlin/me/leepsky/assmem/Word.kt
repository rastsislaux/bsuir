...
/**
 * Perform a binary operation on two words.
 *
 * @param word the second word to perform an operation with
 * @param operation a functor that is a definition of operation
 */
fun Word.operation(word: Word, operation: (Int, Boolean) -> Boolean): Boolean {
    checkWord(size, word)
    return mapIndexed(operation).all { it }
}
